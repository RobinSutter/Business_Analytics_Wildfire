...
**Important Notes:**
- Hyperparameter optimization steps are computationally intensive (3-4 hours). Pre-trained models are available in the `modelling_and_prediction/notebooks/models/` directory for immediate use.
- **Model Files**: The trained machine learning model files (`.joblib` files) are too large for GitHub (exceeding the 100MB file size limit) and are not included in this repository. The models include:
  - `ensemble_xgb_model.joblib` (311 MB) - Optimized regression model
  - `risk_classifier_custom_weights.joblib` (83 MB) - Classification model
  - `best_xgb_model.joblib` (64 MB) - Baseline XGBoost model
  
  These models can be generated by running the training notebook, or are available locally in the `modelling_and_prediction/notebooks/models/` directory.
